...
import 'package:project/Core/cubits/specific_entity_income/specific_entity_income_cubit.dart';
import 'package:project/Core/cubits/specific_entity_income/specific_entity_income_state.dart';
import 'package:project/Presentation/mothlyRevenuePage/Widgets/EntityComparsionBarChart.dart';
...
class _ComparePeriodTabState extends State<ComparePeriodTab> {
  ...
  child: PageView.builder(
    ...
    itemBuilder: (context, index) {
      ...
      return EntityComparisonBarChart(
        entityName: item.currEntityName,
        currentIncome: item.currTotalEgpIncome ?? 0,
        previousIncome: item.prevTotalEgpIncome ?? 0,
      );
    },
  ),
);
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(
                  height: 400,
                  child: BlocBuilder<SpecificEntityCubit,
                      SpecificEntityIncomeState>(
                    builder: (context, state) {
                      if (state is SpecificEntityIncomeLoading) {
                        return const Center(
          ...
          BlocBuilder<SpecificEntityCubit, SpecificEntityIncomeState>(
            builder: (context, state) {
              if (state is SpecificEntityIncomeLoaded) {
                ...
                final formattedPrevPeriodTotal =
                    formatter.format(prevPeriodTotal);
